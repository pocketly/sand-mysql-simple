...
module.exports = exports = function(table, staticMethods) {
  ...
  Model.selectRows = function(where, callback) {
    Util.validateProps(where);

    var select = this().select().where(where).toSQL();
   ...
   * @param props
   * @param where
   * @param callback {function} (err, result, row)
   */
  Model.insertOrUpdate = function(props, where, callback) {
    this.selectRow(where, function(err, row) {
      ...
      if (!row) {
        return Model.insert(props, callback);

      } else {
        return Model.update(props, where, callback);
      }
    });
  };
